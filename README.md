# Summer_Assignment_25110CN342

Summer preparation coursework and coding projects
for my college freshman year.

## 👤 Author
**pratik-baghel**
G L Bajaj Institute of Technology and Management

## 📚 About
Daily C programming exercises from the Career
Development Centre Summer Vacation Self Learning
Assignments. Total: 30 Days | 120 Questions

## 📁 Progress

| Folder | Topics | Status |
|--------|--------|--------|
| Day_01 | Sum of N numbers, Multiplication table, Factorial, Count digits | ✅ Done |
| Day_02 | Sum of digits, Reverse number, Product of digits, Palindrome | ✅ Done |
| Day_03 | Prime check, Prime range, GCD, LCM | ✅ Done |
| Day_04 | Fibonacci series, Nth Fibonacci, Armstrong number | ✅ Done |
| Day_05 | Perfect number, Strong number, Factors, Largest prime factor | ✅ Done |
| Day_06 | Decimal-Binary conversion, Set bits, Power without pow() | ✅ Done |
| Day_07 | Recursive factorial, Fibonacci, Sum of digits, Reverse | ✅ Done |
| Day_08 | Half pyramid, Number triangle, Character triangle, Patterns | ✅ Done |
| Day_09 | Reverse star, Number triangle, Character pattern, Hollow square | ✅ Done |
| Day_10 | Star pyramid, Reverse pyramid, Number pyramid, Char pyramid | ✅ Done |
| Day_11 | Functions - Sum, Maximum, Prime, Factorial | 🔲 Pending |
| Day_12 | Functions - Palindrome, Armstrong, Fibonacci, Perfect | 🔲 Pending |
| Day_13 | Arrays - Input, Sum/Avg, Largest/Smallest, Even/Odd | 🔲 Pending |
| Day_14 | Linear search, Frequency, Second largest, Duplicates | 🔲 Pending |
| Day_15 | Reverse array, Rotate left/right, Move zeroes | 🔲 Pending |
| Day_16 | Missing number, Max frequency, Pair sum, Remove duplicates | 🔲 Pending |
| Day_17 | Merge, Union, Intersection of arrays | 🔲 Pending |
| Day_18 | Bubble sort, Selection sort, Binary search | 🔲 Pending |
| Day_19 | Matrix Add, Subtract, Transpose, Diagonal sum | 🔲 Pending |
| Day_20 | Matrix Multiply, Symmetric, Row/Column sum | 🔲 Pending |
| Day_21 | String length, Reverse, Vowels, Case conversion | 🔲 Pending |
| Day_22 | Palindrome string, Word count, Char frequency | 🔲 Pending |
| Day_23 | Non-repeating char, Anagram, Max occurring char | 🔲 Pending |
| Day_24 | String rotation, Compression, Longest word | 🔲 Pending |
| Day_25 | Merge sorted arrays, Common chars, Sort names | 🔲 Pending |
| Day_26 | Guessing game, Voting system, ATM, Quiz app | 🔲 Pending |
| Day_27 | Student, Employee, Salary management systems | 🔲 Pending |
| Day_28 | Library, Bank, Ticket, Contact management | 🔲 Pending |
| Day_29 | Menu-driven Calculator, Array ops, String ops | 🔲 Pending |
| Day_30 | Mini project using arrays, strings & functions | 🔲 Pending |

## 📈 Overall Progress
🟩🟩🟩🟩🟩🟩🟩🟩🟩🟩⬜⬜⬜⬜⬜⬜⬜⬜⬜⬜⬜⬜⬜⬜⬜⬜⬜⬜⬜⬜
10/30 Days Complete | 40/120 Questions Done

## 💻 Language
- C Programming

## 🏢 Target Companies
TCS | Infosys | Wipro | Amazon | Microsoft | Adobe | Paytm | Deloitte | Capgemini | Accenture
